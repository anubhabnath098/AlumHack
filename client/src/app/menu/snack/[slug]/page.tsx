    ...
    <div className='w-full h-screen flex justify-center items-center bg-yellow-100'>
        <div className="h-[80%] w-[50%] rounded bg-white border border-gray-500 flex">
            <div className="w-[50%] flex flex-col justify-center items-center gap-4">
                ...
                {fooditem[0].availability&&(<div className="rounded text-bold border-2 border-green-950 cursor-pointer w-[100px] h-[40px] flex justify-center items-center gap-2 text-red-950"><AddIcon onClick={e=>setQty(qty+1)}/>{qty===0?"Add":`${qty}`}<RemoveIcon onClick={e=>setQty(Math.max(0, qty-1))}/></div>)}
